...
from flyable.data.lang_func_impl import FuncImplType
...
class ParserVisitorAst(NodeVisitor):
    ...
    def __init__(self, parser, code_gen, func_impl):
        ...
        self.__data: CompData = parser.get_data()
    ...
    @property
    def __last_tv_pair(self):
        """
        tv stands for type_value
        :return: the last type value pair in a RefCounter(type, value)
        """
        return ref_counter.RefCounter(self.__last_type, self.__last_value)
    ...
    def visit_UnaryOp(self, node: UnaryOp) -> Any:
        ...
        if isinstance(node.op, ast.Not):
            ...
            self.__last_type, self.__last_value = runtime.value_to_pyobj(self, self.__last_value,
    ...
    def visit_Subscript(self, node: Subscript) -> Any:
        sequence_type, sequence = self.__visit_node(node.value)
        slice_type, slice = self.__visit_node(node.slice)

        if isinstance(node.ctx, ast.Load):
            get_item = self.__code_gen.get_or_create_func("PyObject_GetItem", code_type.get_py_obj_ptr(self.__code_gen),
                                                          [code_type.get_py_obj_ptr(self.__code_gen)] * 2,
                                                          _gen.Linkage.EXTERNAL)
